...
import { z } from "zod";
import { generateContextualAssistantAnswer } from "@/lib/ai/dashboard-assistant";
import { buildContextualAssistantBaseContext } from "@/lib/assistant/dashboard-context";
import type {
...
function filterReferencedRecords(
  ...
  sourceMap: Map<string, ContextualAssistantReferencedRecord>,
) {
  const referencedRecords: ContextualAssistantReferencedRecord[] = [];
...
function parsePreviousMessages(
  ...
  const messages: ContextualAssistantHistoryMessage[] = [];
...
function parsePageContext(
  value: unknown,
): ContextualAssistantPageContextInput | undefined {
...
export async function askContextualAssistant(
  input: ContextualAssistantActionInput,
): Promise<ContextualAssistantActionState> {
  ...
  try {
    const context = await buildContextualAssistantBaseContext({
      userId,
      question: parsedQuestion.data,
      pageContext,
    });
    const answer = await generateContextualAssistantAnswer({
      question: parsedQuestion.data,
